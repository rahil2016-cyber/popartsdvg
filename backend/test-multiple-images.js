...
async function testAddProductWithMultipleImages() {
  try {
    ...
    console.log('Step 3: Creating product with multiple images...');
    const form = new FormData();
    form.append('name', 'Test Product with Multiple Images');
    form.append('slug', 'test-product-multi-' + Date.now());
    form.append('description', 'Test product with multiple images');
    form.append('price', '299.99');
    form.append('stock', '20');
    form.append('featured', 'true');
    form.append('category_id', categoriesRes.data[0].id);

    // Add the test image twice
...
testAddProductWithMultipleImages();
